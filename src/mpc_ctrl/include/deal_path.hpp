...
public:
    ...
    void find_step(int step, Eigen::MatrixXd &A, Eigen::VectorXd &b, double &step_x, double &step_y,double &yaw_r)
    {
        ...
        yaw_r = atan((end_point.second - start_point.second) / (end_point.first - start_point.first));
        if (end_point.first < start_point.first)
        {
            yaw_r += M_PI;
        }
    }
